...
def run_amixer(command_args):
    try:
        ...
        print(f"[VOLUME] amixer control error: {e}")

class VolumePlugin(DialPlugin):
    name = "System Volume"
    description = "Controls system volume with dial, click/tap to toggle mute."
    author = "System"
    version = "1.0.0"
    
    def __init__(self, settings: dict = None):
        super().__init__(settings)
        self.volume = self.settings.get("default_volume", 50)
        self.is_muted = False
    ...
    def draw_segment(self, draw: ImageDraw.ImageDraw, width: int, height: int) -> None:
        ...
        draw.text((15, 10), "VOLUME", fill=(150, 150, 150), font=font_title)
    ...
    def toggle_mute(self):
        ...
        print(f"[VOLUME] Mute state toggled to: {self.is_muted}")
